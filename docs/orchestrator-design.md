...
**Status:** **v0.3 — validated.** Two fresh-context adversarial passes (design + code, both on the live macOS box) plus `orchestrator/test.sh` driving the worker against a stubbed `mxr` — **10/10 green**, including a `set -u` crash the code review missed but the test caught. Stack: **bash client over the UNMODIFIED runtime spine.**
