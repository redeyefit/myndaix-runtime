...
**Status:** **v0.5 — validated; self-review bugs fixed.** Design + code adversarial passes + `orchestrator/test.sh` (stubbed `mxr`) — **14/14 green**. A live kilabz review of this code found 3 real bugs (it reviewed itself); all fixed and pinned by tests. Stack: **bash client over the UNMODIFIED runtime spine.**
