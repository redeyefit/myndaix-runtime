...
def resolve_and_hash(repo_path: str, tip_sha: str, path: str, line: int,
    ...
    """Read the flagged line from git OBJECTS at `tip_sha` (`git show <tip>:<path>`, 1-indexed line
    `line`), validate it falls INSIDE one of this diff's `changed_hunks` for that path, and return its
    ...
    if out is None:                                 # missing object / non-zero git exit -> drop
        return None
    lines = out.split("\n")
