...
# failure is a HARD stop; oracle is a degradable weak backup that gets the REAL fenced diff; a staging
# DEGRADATION is surfaced (not swallowed); raw reviews are printed before synthesis. DESIGN mode leads
# with oracle and degrades (not stops) if oracle is down. Run: bash orchestrator/test-xreview.sh
set -uo pipefail
...
printf '%s\n' '#!/usr/bin/env bash' 'printf "deadbeefcafe1234\n"' > "$FAKE/.local/bin/openssl"; chmod +x "$FAKE/.local/bin/openssl"
...
echo "1. code mode: kilabz GATE runs via 'mxr review' (staged snapshot), oracle gets the REAL fenced diff, verdict printed"; rm -f "$FAKE/mxr-argv.log"
  ...
  ck "$(log)" "BEGIN UNTRUSTED pushed-diff" "oracle backup gets a nonce-fenced diff (finding #3)"
  ...
  # raw reviews printed to stdout BEFORE the synthesized verdict (finding #4) — survive a lobster failure
...
echo "4. code mode: a staging DEGRADATION is surfaced LOUDLY, not swallowed (finding #1)"; rm -f "$FAKE/mxr-argv.log"
...
echo "5. code mode: an unresolvable repo is a usage error (exit 2)"; rm -f "$FAKE/mxr-argv.log"
  run code /no/such/repo "$RANGE" >/dev/null 2>&1; ckx $? 2 "unresolvable repo -> exit 2"

echo "6. design mode: oracle LEADS (dispatched), verdict printed"; rm -f "$FAKE/mxr-argv.log"
...
echo "7. design mode: oracle DOWN degrades (not a hard stop) — still exits 0 with a verdict"; rm -f "$FAKE/mxr-argv.log"
...
echo "8. usage errors exit 2"; run bogus >/dev/null 2>&1; ckx $? 2 "bad mode -> exit 2"
